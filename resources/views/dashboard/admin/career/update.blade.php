@extends('layouts.dashboard.app', ['title' => 'Update Career'])
@section('css')
    <!--quill css-->
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="pe-3" data-simplebar>
                        <form action="{{ route('admin.career.update.save', ['id' => $career->id]) }}" method="POST">
                            <div class="row">
                                @csrf
                                <div class="col-md-12">
                                    <div class="mb-3">
                                        <label class="form-label" for="title">Title</label>
                                        <input type="text" class="form-control" name="title" id="title" placeholder="Career Title*" value="{{ old('title') ?? $career->title }}">
                                        @error('title')
                                    ...
                                    <label for="mode" class="col-md-12 col-form-label">Mode of Employment</label>
                                        <select name="mode" class="form-select">
                                            ...
                                            @foreach ($modes as $mode)
                                                <option @if ((old('mode') == $mode) || ($career->mode == $mode)) selected @endif value="{{ $mode }}">{{ $mode }}</option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="col-md-12">
                                    <div class="mb-3">
                                        ...
                                        <input type="number" class="form-control" name="min" id="min" placeholder="Career Min. Salary*" value="{{ old('min') ?? $career->min }}">
                                        @error('min')
                                            <span class="text-danger">{{ $message }}</span>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="number" class="form-control" name="max" id="max" placeholder="Career Max. Salary*" value="{{ old('max') ?? $career->max }}">
                                        @error('max')
                                            <span class="text-danger">{{ $message }}</span>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control" name="fee" id="fee" placeholder="Career Applocation Fee*" value="{{ old('fee') ?? $career->fee }}">
                                        @error('fee')
                                            <span class="text-danger">{{ $message }}</span>
                                        @enderror
                                    </div>
                                ...
                                <div class="col-md-12 mb-1">
                                    ...
                                    <textarea class="form-control" name="description" id="elm2" placeholder="Job Description..." rows="3">{{ empty(old('description')) ? $career->description : old('description') }}</textarea>
                                    @error('description')<span class="text-danger">{{ $message }}</span>@enderror
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-6 text-center mx-auto col-12">
                                    <button type="submit" class="btn btn-primary" type="submit">Update Job</button>
                                </div>
                            </div>
                        </form>
                    </div>
