@extends('layouts.dashboard.app', ['title' => 'Edit a Category'])
@section('css')
    <!--quill css-->
                ...
                <div class="col-xl-8 col-lg-8 mx-auto col-md-8">
                    <div class="card">
                        <div class="card-body p-4">
                            <div class="">
                                <!-- end row -->
                                <h4 class="font-size-18 text-muted mt-1 text-center">Edit a Category</h4>
                                <p class="mb-3 text-center">Update a new Category for Blog Post</p>
                                <form class="form-horizontal" method="POST" action="{{ route('admin.categories.edit.save') }}">
                                    @csrf
                                    <div class="row">
                                <input type="hidden" name="id" value="{{ $category->id }}">
                                        <div class="col-md-12 mb-1">
                                            <label for="type" class="col-md-12 col-form-label">Category Title</label>
                                            <input class="form-control" name="title" type="text" placeholder="Category Title..." value="{{ old('title') ?? $category->title }}" id="title">
                                            @error('title')<span class="text-danger">{{ $message }}</span>@enderror
                                        </div>

                                        <div class="col-md-12 mb-1">
                                            <label for="type" class="col-md-12 col-form-label">Body of Category</label>
                                            <textarea class="form-control" name="body" id="email-editor" placeholder="Body of Category..." rows="3">{{ old('body') ?? $category->body }}</textarea>
