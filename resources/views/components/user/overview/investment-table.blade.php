                                            ...
                                            <tbody>
                                                @foreach ($investments() as $data)
                                                    <tr>
                                                        <td>
                                                            <div class="avatar-xs">
                                                                <span class="avatar-title rounded-circle bg-primary text-white" style="padding: 7px 10px;  margin-top: 10px !important;">
                                                                    {{  Str::upper(Str::substr($data->user->username, 0, 1)) }}
                                                                </span>
                                                            </div>
                                                        </td>
                                                        <td>
                                                            <p class="font-size-12 mb-0">#{{  $data->transaction_id }}</p>
                                                            <h6 class="font-size-15 my-0">{{ $data->user->username }}</h6>
                                                        </td>
                                                        <td style="white-space: nowrap">{{ date('j D M, Y', strtotime($data->date)) }}</td>
                                                        <td>${{ $data->amount }}</td>
                                                        {{-- <td>{{ $data->plan->type }}</td> --}}
                                                        <td style="white-space: nowrap">{{ $data->plan->name }} : <span class="text-success">{{ $data->plan->rate }}%</span></td>
                                                        ...
                                                        <td>
                                                            @if ($data->status)
                                                                <a href="{{ route('user.transaction.withdraw.new') }}" class="btn btn-outline-success btn-sm"><i class="mdi mdi-bank-minus"></i> Withdraw</a>
                                                                {{-- @if (!$data->reinvest) --}}
                                                                    <a 
                                                                        href="{{ route('reinvest', ['transaction_id' => $data->transaction_id, 'type' => 'all']) }}" 
                                                                        class="btn btn-outline-success{{ ($data->user->balance < ($data->numAmount+$data->earning)) ? ' disabled' : '' }} btn-sm"
                                                                    >
                                                                    ...
                                                                    <a 
                                                                        href="{{ route('reinvest', ['transaction_id' => $data->transaction_id, 'type' => 'principal']) }}" 
                                                                        class="btn btn-outline-success{{ ($data->user->balance < $data->numAmount) ? ' disabled' : '' }} btn-sm"
                                                                    >
                                                                        <i class="mdi mdi-reload me-2"></i> 
                                                                        Re Invest Principal
                                                                    </a>
                                                                {{-- @endif --}}
                                                            @elseif(!$data->status && $data->paused)
                                                                Investment Paused <a href="{{ route('user.contact.new') }}" class="btn btn-outline-info btn-sm">Our Support</a>
                                                            @else
                                                                <a href="#" class="btn blink_me btn-disabled btn-outline-success btn-sm">Running</a>
                                                            @endif
                                                            @if($data->user->admin)
                                                                <a href="{{ route('admin.investment.pause', ['transaction_id' => $data->transaction_id]) }}" class="btn btn-outline-info btn-sm">{{ $data->paused ? 'UnPause' : 'Pause' }}</a>
                                                            ...
                                                            <div class="row bg-soft-secondary">
                                                                <div class="col-md-12 d-flex titlep">
                                                                    ...
                                                                    <div class="details child d-flex py-1 pr-2">
                                                                        <p class="time mx-1" style="align-self: center;">
                                                                            {{ $data->time_passed }}</p>
